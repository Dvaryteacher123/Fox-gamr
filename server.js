...
app.post('/api/pay-fimipay', async (req, res) => {
    try {
        ...
        if (!apiKey) {
            return res.status(500).json({ success: false, error: 'Ufunguo wa FimiPay (API Key) haujapatikana kwenye faili la .env.' });
        }

        console.log(`Inatuma ombi la malipo FimiPay kwenda namba: ${formattedPhone} kwa kiasi cha: ${amount}`);

        // Ombi kwenda FimiPay API kulingana na docs zao
        const fimiResponse = await fetch('https://fimipay.com/api/v1/collect', {
            method: 'POST',
            headers: {
