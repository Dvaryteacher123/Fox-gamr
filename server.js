...
// ⚠️ MUHIMU: webhook endpoint inahitaji RAW body kwa signature verification
// Tunatumia `verify` option ili kuhifadhi raw body kwenye req.rawBody
app.use(express.json({
    limit: '1mb',
    verify: (req, res, buf) => {
        // Hifadhi raw body kwa routes zote (tunaitumia kwenye webhook)
        req.rawBody = buf;
    }
}));
...
function verifyFimiPaySignature(rawBody, signature) {
    ...
    if (!secret) {
        console.warn('⚠️ FIMIPAY_WEBHOOK_SECRET haipo — kuruka verification (HATARI!)');
        return true; // Kama hakuna secret, ruhusu (bora uweke!)
    }
    if (!signature) {
        return false;
    }
    ...
    // Linganisha kwa usalama (constant-time comparison)
    try {
...
app.post('/api/pay-fimipay', async (req, res) => {
    try {
        ...
        // ---------- HIFADHI KWA FIRESTORE (status: pending) ----------
        ...
        const fimipayResponse = await fetch(
            'https://fimipay.com/api/v1/collections',
            {
                ...
                body: JSON.stringify({
                    ...
                    callback_url: 'https://dvary.space/api/fimipay/callback'
                })
            }
        );
...
// ============================================================
// FIMIPAY CALLBACK / WEBHOOK → SASISHA FIRESTORE + FUNGUA GAME
// ============================================================
app.post('/api/fimipay/callback', async (req, res) => {
    try {
        const signature = req.headers['x-fimipay-signature'] || req.headers['x-fimipay-signature'.toLowerCase()];
        ...
        if (isSuccess && payment.userId && payment.gameId) {
            ...
            console.warn(`⚠️ Success lakini userId au gameId haipo: userId=${payment.userId}, gameId=${payment.gameId}`);
        }
...
app.listen(PORT, () => {
    ...
    console.log('   POST /api/fimipay/callback  ← FimiPay webhook');
