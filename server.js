...
// ============================================================
// API ENDPOINT - FIMIPAY PAYMENT (Safeguarded against HTML responses & Phone formats)
// ============================================================
app.post('/api/pay-fimipay', async (req, res) => {
    try {
        let { buyer_email, buyer_phone, amount, gameId } = req.body;
        ...
        if (!buyer_email) {
            buyer_email = `user_${Date.now()}@dvarygames.com`;
        }

        // Usahihishaji sahihi wa namba ya simu kwa viwango vya Tanzania (255)
        ...
        if (formattedPhone.startsWith('0')) {
            ...
            // Marekebisho ikiwa iliingia 225 kimakosa
        ...
        // Kuandaa mwili wa ombi (Request Body)
        ...
        // Weka test_outcome KAMA NA KAMA TU funguo inaanza na sk_test_
        if (apiKey.startsWith('sk_test_')) {
            paymentPayload.test_outcome = "success";
        }
        ...
        // Soma majibu kama maandishi (text) kwanza kuzuia SyntaxError ya HTML
        ...
        try {
            ...
            return res.status(500).json({ 
                success: false, 
                error: 'API ya FimiPay haipatikani au URL imekosewa (Imerudisha HTML badala ya JSON).' 
            });
        }
        ...
        if (paymentData.status === "success" || paymentData.success === true) {
            ...
            res.status(400).json({ 
                success: false, 
                error: paymentData.message || paymentData.error || 'Imeshindikana kuchakata malipo.' 
            });
        }
