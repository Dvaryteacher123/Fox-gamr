    ...
    try {
        ...
        const fimipayResponse = await fetch('https://api.fimipay.com/v1/payments', {
            ...
            headers: {
                ...
                'Authorization': 'Bearer sk_test_WAKATI_WA_KUBADILISHA_WEKA_KEY_YAKO' // Badilisha uweke test key halisi ya FimiPay
            },
            body: JSON.stringify({
        ...
        if (paymentData.status === "success") {
            ...
            res.status(400).json({ success: false, error: paymentData.message || 'Imeshindikana kuchakata malipo.' });
        }

    } catch (error) {
        console.error('FimiPay Server Error:', error);
        res.status(500).json({ success: false, error: 'Kosa la kimtandao la seva wakati wa malipo.' });
    }
});
