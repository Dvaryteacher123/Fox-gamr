...
app.post('/api/pay-speedpesa', async (req, res) => {
    try {
        ...
        const speedPesaResponse = await fetch('https://sandbox.speedpesapro.com/api/v1/collections/ussd-push', {
            ...
            body: JSON.stringify({
                msisdn: formattedPhone, // Imetumia msisdn sahihi yenye 's' mbili
        ...
        if (speedPesaResponse.ok && (paymentData.success || paymentData.status === 'success' || paymentData.status === 'PENDING' || paymentData.status === 'SUCCESS')) {
            res.json({
                success: true,
            ...
            console.error("SpeedPesa Error Details:", paymentData);
            res.status(400).json({ 
                success: false, 
                error: paymentData.message || paymentData.error || 'Imeshindwa kuchakata malipo kupitia SpeedPesa.' 
            });
        }
