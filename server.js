...
// ============================================================
// API ENDPOINT - SPEEDPESA PAYMENT (NATIVE HTTPS)
// ============================================================
app.post('/api/pay-speedpesa', async (req, res) => {
    try {
        ...
        const options = {
            hostname: 'sandbox.speedpesapro.com',
            port: 443,
            path: '/api/v1/collections/ussd-push',
            method: 'POST',
            headers: {
        ...
        const speedPesaReq = new Promise((resolve, reject) => {
            const reqOut = https.request(options, (resOut) => {
                let responseBody = '';
                resOut.on('data', (chunk) => responseBody += chunk);
                resOut.on('end', () => {
                    try {
                        resolve({ statusCode: resOut.statusCode, body: JSON.parse(responseBody) });
                    } catch (e) {
                        resolve({ statusCode: resOut.statusCode, body: { message: responseBody } });
                    }
                });
            });

            reqOut.on('error', (error) => reject(error));
            reqOut.write(data);
            reqOut.end();
        });

        const result = await speedPesaReq;
        console.log("Majibu ya SpeedPesa:", result);

        if (result.statusCode >= 200 && result.statusCode < 300) {
            res.json({
                success: true,
                message: result.body.message || "Ombi la malipo limetumwa kwenye simu.",
                data: result.body
            });
        } else {
            res.status(400).json({ 
                success: false, 
                error: result.body.message || result.body.error || 'Imeshindwa kuchakata malipo kupitia SpeedPesa.' 
            });
        }
