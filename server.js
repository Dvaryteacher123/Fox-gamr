...
// ============================================================
// API ENDPOINT - FIMIPAY LIVE PAYMENT 
// ============================================================
app.post('/api/pay-fimipay', async (req, res) => {
    try {
        ...
        if (!fimipayKey) {
            return res.status(500).json({ success: false, error: 'FimiPay Live API Key haipatikani kwenye .env.' });
        }

        console.log(`Inatuma ombi la LIVE la FimiPay kwenda namba: ${formattedPhone} kwa kiasi: ${amount}`);

        // Tumia api.fimipay.com na headers za ziada ili kukwepa Cloudflare bot block
        const fimiResponse = await fetch('https://api.fimipay.com/api/v1/collections', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'User-Agent': 'DVARY-GAMES-Server/1.0',
        ...
        try {
            ...
            console.error("FimiPay imerudisha majibu yasiyo ya JSON (Cloudflare/HTML):", responseText);
            return res.status(500).json({ success: false, error: 'Hitilafu ya kupokea majibu kutoka FimiPay (Cloudflare blockage).' });
        }
